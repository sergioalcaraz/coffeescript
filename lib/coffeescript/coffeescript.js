...
(function() {
  ...
  var {getSourceMap, registerCompiled} = SourceMap;
  ...
  exports.compile = compile = withPrettyErrors(function(code, options = {}) {
    ...
    var filename = options.filename || helpers.anonymousFileName();
    ...
    if (options.ast) {
      ...
      var sourceCodeNumberOfLines = (code.match(/\r?\n/g) || '').length + 1;
      var sourceCodeLastLine = /.*$/.exec(code)[0];
      var ast = nodes.ast(options);
      var range = [0, code.length];
    ...
    var fragments = nodes.compileToFragments(options);
    var currentLine = 0;
    if (options.header) {
      currentLine += 1;
    }
    ...
    if (generateSourceMap) {
      var v3SourceMap = map.generate(options, code);
    }
    if (options.transpile) {
      ...
      var transpiler = options.transpile.transpile;
      delete options.transpile.transpile;
      var transpilerOptions = Object.assign({}, options.transpile);
      ...
      var transpilerOutput = transpiler(js, transpilerOptions);
    ...
    if (options.inlineMap) {
      ...
      var sourceURL = `//# sourceURL=${filename}`;
      js = `${js}\n${sourceMapDataURI}\n${sourceURL}`;
    }
  ...
  parser.yy.parseError = function(message, {token}) {
    ...
    var {errorToken, tokens} = parser;
    var [errorTag, errorText, errorLoc] = errorToken;
    errorText = (function() {
      switch (false) {
  ...
  exports.patchStackTrace = function() {
    ...
    var getSourceMapping = function(filename, line, column) {
      var sourceMap = getSourceMap(filename, line, column);
      if (sourceMap != null) {
        var answer = sourceMap.sourceLocation([line - 1, column - 1]);
      }
      if (answer != null) {
  ...
  var checkShebangLine = function(file, input) {
    var ref;
    var firstLine = input.split(/$/m, 1)[0];
    var rest = firstLine != null ? firstLine.match(/^#!\s*([^\s]+\s*)(.*)/) : void 0;
    var args = rest != null ? (ref = rest[2]) != null ? ref.split(/\s/).filter(function(s) {
      return s !== '';
    }) : void 0 : void 0;
