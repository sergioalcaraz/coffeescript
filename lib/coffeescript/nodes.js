...
(function() {
  ...
  var {isUnassignable, JS_FORBIDDEN} = require('./lexer');

  // Import the helpers we plan to use.
  var {compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, replaceUnicodeCodePointEscapes, isFunction, isPlainObject, isNumber, parseNumber} = require('./helpers');
  ...
  var fragmentsToText = function(fragments) {
    ...
    return ((function() {
      var results1 = [];
      for (var j = 0, len1 = fragments.length; j < len1; j++) {
  ...
  exports.Base = Base = (function() {
    class Base {
      ...
      compileToFragments(o, lvl) {
        ...
        node.tab = o.indent;
        var fragments = o.level === LEVEL_TOP || !node.isStatement(o) ? node.compileNode(o) : node.compileClosure(o);
      ...
      compileCommentFragments(o, node, fragments) {
        ...
        // later on.
        var unshiftCommentFragment = function(commentFragment) {
          if (commentFragment.unshift) {
            ...
            if (fragments.length !== 0) {
              var precedingFragment = fragments[fragments.length - 1];
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (comment.here) { // Block comment, delimited by `###`.
            var commentFragment = new HereComment(comment).compileNode(o); // Line comment, delimited by `#`.
          } else {
      ...
      cache(o, level, shouldCache) {
        var complex = shouldCache != null ? shouldCache(this) : this.shouldCache();
        if (complex) {
      ...
      hoist() {
        this.hoisted = true;
        var target = new HoistTarget(this);
        var compileNode = this.compileNode;
        var compileToFragments = this.compileToFragments;
      ...
      eachChild(func) {
        ...
        var ref1 = this.children;
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var attr = ref1[j];
          if (this[attr]) {
            var ref2 = flatten([this[attr]]);
            for (var k = 0, len2 = ref2.length; k < len2; k++) {
              var child = ref2[k];
              if (func(child) === false) {
                return this;
              }
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (children = this[attr]) {
            if (Array.isArray(children)) {
              for (var k = 0, i = k, len2 = children.length; k < len2; i = ++k) {
      ...
      joinFragmentArrays(fragmentsList, joinStr) {
        var answer = [];
        for (var j = 0, i = j, len1 = fragmentsList.length; j < len1; i = ++j) {
    ...
    Base.prototype.isDeclarable = NO; // could this be the left-hand side in a `var` declaration?
  ...
  exports.HoistTarget = HoistTarget = class HoistTarget extends Base {
    ...
    static expand(fragments) {
      var ref1;
      for (var j = fragments.length - 1, i = j; j >= 0; i = j += -1) {
  ...
  exports.Root = Root = (function() {
    class Root extends Base {
      ...
      compileNode(o) {
        ...
        var fragments = this.body.compileRoot(o);
        if (o.bare) {
          return fragments;
        }
        var functionKeyword = `${this.isAsync ? 'async ' : ''}function`;
      ...
      initializeScope(o) {
        ...
        var ref2 = o.locals || [];
        var results1 = [];
        for (var j = 0, len1 = ref2.length; j < len1; j++) {
          var name = ref2[j];
      ...
      commentsAst() {
        ...
        if (this.allComments == null) {
          this.allComments = (function() {
            var ref1;
            var ref2 = (ref1 = this.allCommentTokens) != null ? ref1 : [];
            var results1 = [];
            for (var j = 0, len1 = ref2.length; j < len1; j++) {
              commentToken = ref2[j];
  ...
  exports.Block = Block = (function() {
    class Block extends Base {
      ...
      makeReturn(results, mark) {
        var expressions, ref1, ref2;
        var len = this.expressions.length;
        ref1 = this.expressions, [lastExp] = slice1.call(ref1, -1);
        var lastExp = (lastExp != null ? typeof lastExp.unwrap === "function" ? lastExp.unwrap() : void 0 : void 0) || false;
        ...
        if (lastExp && lastExp instanceof Parens && lastExp.body.expressions.length > 1) {
          ...
          var penult = penult.unwrap();
          var last = last.unwrap();
      ...
      compileNode(o) {
        ...
        var compiledNodes = [];
        var ref1 = this.expressions;
        for (var j = 0, index = j, len1 = ref1.length; j < len1; index = ++j) {
          var node = ref1[index];
          if (node.hoisted) {
        ...
        if (compiledNodes.length) {
          var answer = this.joinFragmentArrays(compiledNodes, ', ');
        } else {
      ...
      compileWithDeclarations(o) {
        ...
        var post = [];
        var ref1 = this.expressions;
        for (var j = 0, i = j, len1 = ref1.length; j < len1; i = ++j) {
          var exp = ref1[i];
          exp = exp.unwrap();
        ...
        if (scope.expressions === this) {
          if (scope.hasDeclarations()) {
            ...
            var unassigned = scope.declaredVariables(false);
            var assigned = scope.declaredVariables(true);
            var declare = (names, sep) => {
              var k, len2;
              var results1 = [];
              for (k = 0, i = k, len2 = names.length; k < len2; i = ++k) {
                var name = names[i];
                var v = scope.get(name);
                fragments.push(this.makeCode(v.name));
                if (v.comments != null) {
                  fragments.push(...v.comments);
                }
                if (v.assigned != null) {
                  fragments.push(this.makeCode(` = ${v.assigned}`));
                }
                if (i !== names.length - 1) {
                  results1.push(fragments.push(this.makeCode(sep)));
                } else {
      ...
      compileComments(fragments) {
        var commentFragment;
        for (var j = 0, fragmentIndex = j, len1 = fragments.length; j < len1; fragmentIndex = ++j) {
          ...
          if (fragment.precedingComments) {
            ...
            var fragmentIndent = '';
            var ref1 = fragments.slice(0, (fragmentIndex + 1));
            for (var k = ref1.length - 1; k >= 0; k += -1) {
              var pastFragment = ref1[k];
              var indent = /^ {2,}/m.exec(pastFragment.code);
              if (indent) {
            ...
            var code = `\n${fragmentIndent}` + ((function() {
              var ref2 = fragment.precedingComments;
              var results1 = [];
              for (var l = 0, len2 = ref2.length; l < len2; l++) {
            ...
            var ref2 = fragments.slice(0, (fragmentIndex + 1));
            for (var l = ref2.length - 1, pastFragmentIndex = l; l >= 0; pastFragmentIndex = l += -1) {
              pastFragment = ref2[pastFragmentIndex];
              var newLineIndex = pastFragment.code.lastIndexOf('\n');
              if (newLineIndex === -1) {
          ...
          if (fragment.followingComments) {
            ...
            var trail = fragment.followingComments[0].trail;
            fragmentIndent = '';
            ...
            if (!(trail && fragment.followingComments.length === 1)) {
              ...
              var ref3 = fragments.slice(fragmentIndex);
              for (var p = 0, len2 = ref3.length; p < len2; p++) {
                var upcomingFragment = ref3[p];
                if (!onNextLine) {
            ...
            code += ((function() {
              var ref4 = fragment.followingComments;
              var results1 = [];
            ...
            var ref4 = fragments.slice(fragmentIndex);
            for (var q = 0, upcomingFragmentIndex = q, len3 = ref4.length; q < len3; upcomingFragmentIndex = ++q) {
      ...
      astProperties(o) {
        var checkForDirectives = del(o, 'checkForDirectives');
        ...
        var ref1 = this.expressions;
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var expression = ref1[j];
          var expressionAst = expression.ast(o);
  ...
  exports.StringLiteral = StringLiteral = class StringLiteral extends Literal {
    constructor(originalValue, {
      ...
      var heredoc = this.isFromHeredoc();
    ...
    withoutQuotesInLocationData() {
      var endsWithNewline = this.originalValue.slice(-1) === '\n';
      var locationData = Object.assign({}, this.locationData);
      ...
      var copy = new StringLiteral(this.originalValue, {quote: this.quote, initialChunk: this.initialChunk, finalChunk: this.finalChunk, indent: this.indent, double: this.double, heregex: this.heregex});
  ...
  exports.RegexLiteral = RegexLiteral = (function() {
    class RegexLiteral extends Literal {
      constructor(value, {delimiter: delimiter1 = '/', heregexCommentTokens: heregexCommentTokens = []} = {}) {
        ...
        var heregex = this.delimiter === '///';
        var endDelimiterIndex = value.lastIndexOf('/');
        this.flags = value.slice(endDelimiterIndex + 1);
        var val = this.originalValue = value.slice(1, endDelimiterIndex);
        if (heregex) {
          val = val.replace(HEREGEX_OMIT, '$1$2');
        }
      ...
      astProperties(o) {
        var heregexCommentToken;
        var [, pattern] = this.REGEX_REGEX.exec(this.value);
        return {
          value: void 0,
          pattern,
          flags: this.flags,
          ...
          comments: (function() {
            var ref1 = this.heregexCommentTokens;
            var results1 = [];
            for (var j = 0, len1 = ref1.length; j < len1; j++) {
  ...
  exports.IdentifierLiteral = IdentifierLiteral = (function() {
    class IdentifierLiteral extends Literal {
      isDeclarable(o) {
        ...
        return o.scope.get(this.value) != null;
      }
  ...
  exports.ThisLiteral = ThisLiteral = class ThisLiteral extends Literal {
    ...
    compileNode(o) {
      var ref1;
      var code = ((ref1 = o.scope.method) != null ? ref1.bound : void 0) ? o.scope.method.context : this.value;
  ...
  exports.Return = Return = (function() {
    class Return extends Base {
      ...
      compileToFragments(o, level) {
        var ref1;
        var expr = (ref1 = this.expression) != null ? ref1.makeReturn() : void 0;
...
// multiline need to be indented.
          for (var j = 0, len1 = answer.length; j < len1; j++) {
            var fragment = answer[j];
  ...
  exports.Value = Value = (function() {
    class Value extends Base {
      ...
      isDeclarable(o) {
        return this.properties.length === 0 && this.base.isDeclarable(o);
      }
      ...
      isAtomic() {
        var ref1 = this.properties.concat(this.base);
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var node = ref1[j];
      ...
      cacheReference(o) {
        ...
        if (base.shouldCache()) { // `a().b`
          var bref = new IdentifierLiteral(o.scope.freeVariable('base'));
        ...
        if (name.shouldCache()) { // `a[b()]`
          var nref = new IdentifierLiteral(o.scope.freeVariable('name'));
          var name = new Index(new Assign(nref, name.index));
          nref = new Index(nref);
        }
      ...
      compileNode(o) {
        ...
        for (var j = 0, len1 = props.length; j < len1; j++) {
          var prop = props[j];
      ...
      unfoldSoak(o) {
        return this.unfoldedSoak != null ? this.unfoldedSoak : this.unfoldedSoak = (() => {
          var ifn = this.base.unfoldSoak(o);
          if (ifn) {
            ifn.body.properties.push(...this.properties);
            return ifn;
          }
          var ref1 = this.properties;
          for (var j = 0, i = j, len1 = ref1.length; j < len1; i = ++j) {
            var prop = ref1[i];
            if (!prop.soak) {
              continue;
            ...
            prop.soak = false;
            var fst = new Value(this.base, this.properties.slice(0, i));
            var snd = new Value(this.base, this.properties.slice(i));
            if (fst.shouldCache()) {
              var ref = new IdentifierLiteral(o.scope.freeVariable('ref'));
      ...
      object() {
        ...
        var initialProperties = this.properties.slice(0, this.properties.length - 1);
        // Create the `object` that becomes the new “base” for the split-off final
        // property.
        var object = new Value(this.base, initialProperties, this.tag, this.isDefaultValue);
      ...
      astProperties(o) {
        ...
        var computed = property instanceof Index || !(((ref2 = property.name) != null ? ref2.unwrap() : void 0) instanceof PropertyName);
        return {
  ...
  exports.HereComment = HereComment = class HereComment extends Base {
    ...
    compileNode(o) {
      var multiline = indexOf.call(this.content, '\n') >= 0;
      // Unindent multiline comments. They will be reindented later.
      if (multiline) {
        var indent = null;
        var ref1 = this.content.split('\n');
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var line = ref1[j];
          var leadingWhitespace = /^\s*/.exec(line)[0];
      ...
      var fragment = this.makeCode(this.content);
  ...
  exports.LineComment = LineComment = class LineComment extends Base {
    ...
    compileNode(o) {
      var fragment = this.makeCode(/^\s*$/.test(this.content) ? '' : `${this.precededByBlankLine ? `\n${o.indent}` : ''}//${this.content}`);
      fragment.newLine = this.newLine;
      fragment.unshift = this.unshift;
  ...
  exports.JSXAttribute = JSXAttribute = (function() {
    class JSXAttribute extends Base {
      ...
      compileNode(o) {
        var compiledName = this.name.compileToFragments(o, LEVEL_LIST);
        if (this.value == null) {
          return compiledName;
        }
  ...
  exports.JSXAttributes = JSXAttributes = (function() {
    class JSXAttributes extends Base {
      constructor(arr) {
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (base instanceof IdentifierLiteral) {
            ...
            var attribute = new JSXAttribute({
            ...
            var ref2 = base.properties;
            // Obj containing attributes with values eg a="b" c={d}
            for (var k = 0, len2 = ref2.length; k < len2; k++) {
              var property = ref2[k];
              var {variable, value} = property;
      ...
      checkValidAttribute(object) {
        ...
        var properties = (attribute != null ? attribute.properties : void 0) || [];
      ...
      compileNode(o) {
        var fragments = [];
        var ref1 = this.attributes;
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var attribute = ref1[j];
          fragments.push(this.makeCode(' '));
  ...
  exports.JSXNamespacedName = JSXNamespacedName = (function() {
    class JSXNamespacedName extends Base {
      constructor(tag) {
        super();
        var [namespace, name] = tag.value.split(':');
  ...
  exports.JSXElement = JSXElement = (function() {
    class JSXElement extends Base {
      ...
      elementAstProperties(o) {
        ...
        if (this.closingElementLocationData != null) {
          ...
          if ((ref1 = closingElement.name.type) === 'JSXMemberExpression' || ref1 === 'JSXNamespacedName') {
            var rangeDiff = closingElement.range[0] - openingElement.range[0] + '/'.length;
            var columnDiff = closingElement.loc.start.column - openingElement.loc.start.column + '/'.length;
            var shiftAstLocationData = (node) => {
            ...
            if (closingElement.name.type === 'JSXMemberExpression') {
              var currentExpr = closingElement.name;
      ...
      fragmentAstProperties(o) {
        var openingFragment = Object.assign({
          type: 'JSXOpeningFragment'
        }, this.openingElementLocationData);
        var closingFragment = Object.assign({
    ...
    class Call extends Base {
      ...
      updateLocationDataIfMissing(locationData) {
        ...
        if (this.locationData && this.needsUpdatedStartLocation) {
          ...
          var base = ((ref1 = this.variable) != null ? ref1.base : void 0) || this.variable;
      ...
      unfoldSoak(o) {
        if (this.soak) {
          if (this.variable instanceof Super) {
            var left = new Literal(this.variable.compile(o));
            var rite = new Value(left);
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          var ifn = unfoldSoak(o, call, 'variable');
        }
        return ifn;
      }
      ...
      compileNode(o) {
        ...
        // `a(x = 5); b(-> x = 6)`
        // (see issue #4437, https://github.com/jashkenas/coffeescript/issues/4437)
        var varAccess = ((ref2 = this.variable) != null ? (ref3 = ref2.properties) != null ? ref3[0] : void 0 : void 0) instanceof Access;
        var argCode = (function() {
          var ref4 = this.args || [];
          var results1 = [];
          for (var j = 0, len1 = ref4.length; j < len1; j++) {
            arg = ref4[j];
        ...
        if (argCode.length > 0 && varAccess && !this.variable.base.cached) {
          var [cache] = this.variable.base.cache(o, LEVEL_ACCESS, function() {
            return false;
          });
          this.variable.base.cached = cache;
        }
        var ref4 = this.args;
        for (var j = 0, argIndex = j, len1 = ref4.length; j < len1; argIndex = ++j) {
          var arg = ref4[argIndex];
          if (argIndex) {
  ...
  exports.SuperCall = SuperCall = (function() {
    class SuperCall extends Call {
      ...
      compileNode(o) {
        ...
        var superCall = new Literal(fragmentsToText(super.compileNode(o)));
        var replacement = new Block(this.expressions.slice());
        if (o.level > LEVEL_TOP) {
          // If we might be in an expression we need to cache and return the result
          var [superCall, ref] = superCall.cache(o, null, YES);
    ...
    class Super extends Base {
      ...
      compileNode(o) {
        ...
        if (!((method.ctor != null) || (this.accessor != null))) {
          var {name, variable} = method;
          if (name.shouldCache() || (name instanceof Index && name.index.isAssignable())) {
            var nref = new IdentifierLiteral(o.scope.parent.freeVariable('name'));
        ...
        if ((ref1 = this.accessor) != null ? (ref2 = ref1.name) != null ? ref2.comments : void 0 : void 0) {
          ...
          var salvagedComments = this.accessor.name.comments;
          delete this.accessor.name.comments;
        }
        var fragments = (new Value(new Literal('super'), this.accessor ? [this.accessor] : [])).compileToFragments(o);
        if (salvagedComments) {
    ...
    class RegexWithInterpolations extends Base {
      ...
      astProperties(o) {
        ...
        return {
          ...
          comments: (function() {
            var ref3 = this.heregexCommentTokens;
            var results1 = [];
  ...
  exports.Range = Range = (function() {
    class Range extends Base {
      ...
      compileVariables(o) {
        ...
        var shouldCache = del(o, 'shouldCache');
      ...
      compileNode(o) {
        ...
        if (o.declare) { // from compileArray: need to declare all variables as local
          ...
          var canDeclare = (o.scope.get(idx) != null) && (!namedIndex || (o.scope.get(idxName) != null));
          ...
          if (canDeclare) {
            if (o.scope.laterVar(idx)) {
              declare = true;
            }
            if (namedIndex && o.scope.laterVar(idxName)) {
              declare = true;
            }
            if (declare) {
              if (this.fromC !== this.fromVar) {
                o.scope.laterVar(this.fromVar);
              }
              if (this.toC !== this.toVar) {
                o.scope.laterVar(this.toVar);
              }
              if (this.stepC !== this.stepVar) {
                o.scope.laterVar(this.stepVar);
              }
            }
          }
        }
        var varPart = declare ? "var " : '';
        if (this.fromC !== this.fromVar) {
          varPart += `${this.fromC}, ${idx} = ${this.fromVar}`;
        } else {
        ...
        var stepNotZero = `${(ref1 = this.stepNum) != null ? ref1 : this.stepVar} !== 0`;
        var stepCond = `${(ref2 = this.stepNum) != null ? ref2 : this.stepVar} > 0`;
        var lowerBound = `${lt} ${known ? to : this.toVar}`;
        var upperBound = `${gt} ${known ? to : this.toVar}`;
        var condPart = this.step != null ? (this.stepNum != null) && this.stepNum !== 0 ? this.stepNum > 0 ? `${lowerBound}` : `${upperBound}` : `${stepNotZero} && (${stepCond} ? ${lowerBound} : ${upperBound})` : known ? `${from <= to ? lt : gt} ${to}` : `(${this.fromVar} <= ${this.toVar} ? ${lowerBound} : ${upperBound})`;
        var cond = this.stepVar ? `${this.stepVar} > 0` : `${this.fromVar} <= ${this.toVar}`;
        // Generate the step.
      ...
      compileArray(o) {
        ...
        if (known) {
          o.index = i;
          o.declare = true;
          var body = fragmentsToText(this.compileNode(o));
        } else {
  ...
  exports.Slice = Slice = (function() {
    class Slice extends Base {
      ...
      compileNode(o) {
        ...
        var fromCompiled = (from != null ? from.compileToFragments(o, LEVEL_PAREN) : void 0) || [this.makeCode('0')];
        if (to) {
          var compiled = to.compileToFragments(o, LEVEL_PAREN);
          var compiledText = fragmentsToText(compiled);
          if (!(!this.range.exclusive && +compiledText === -1)) {
            var toStr = ', ' + (this.range.exclusive ? compiledText : to.isNumber() ? `${+compiledText + 1}` : (compiled = to.compileToFragments(o, LEVEL_ACCESS), `+${fragmentsToText(compiled)} + 1 || 9e9`));
          }
        }
    ...
    class Obj extends Base {
      ...
      isAssignable(opts) {
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          var message = isUnassignable(prop.unwrapAll().value);
          if (message) {
            prop.error(message);
          }
      ...
      reorderProperties() {
        var props = this.properties;
        var splatProps = this.getAndCheckSplatProps();
        var splatProp = props.splice(splatProps[0], 1);
      ...
      compileNode(o) {
        ...
        var lastNode = this.lastNode(this.properties);
        // If this object is the left-hand side of an assignment, all its children
        // are too.
        this.propagateLhs();
        var isCompact = true;
        var ref1 = this.properties;
        for (var k = 0, len2 = ref1.length; k < len2; k++) {
          var prop = ref1[k];
        ...
        for (var l = 0, i = l, len3 = props.length; l < len3; i = ++l) {
          prop = props[i];
          var join = i === props.length - 1 ? '' : isCompact ? ', ' : prop === lastNode ? '\n' : ',\n';
          var indent = isCompact ? '' : idt;
          var key = prop instanceof Assign && prop.context === 'object' ? prop.variable : prop instanceof Assign ? (!this.lhs ? prop.operatorToken.error(`unexpected ${prop.operatorToken.value}`) : void 0, prop.variable) : prop;
          ...
          if (key === prop) {
            if (prop.shouldCache()) {
              var [key, value] = prop.base.cache(o);
      ...
      getAndCheckSplatProps() {
        ...
        var props = this.properties;
        var splatProps = (function() {
          var j, len1;
          var results1 = [];
          for (j = 0, i = j, len1 = props.length; j < len1; i = ++j) {
            prop = props[i];
            if (prop instanceof Splat) {
              results1.push(i);
            }
      ...
      expandProperty(property) {
        var {variable, context, operatorToken} = property;
        var key = property instanceof Assign && context === 'object' ? variable : property instanceof Assign ? (!this.lhs ? operatorToken.error(`unexpected ${operatorToken.value}`) : void 0, variable) : property;
        if (key instanceof Value && key.hasProperties()) {
      ...
      propagateLhs(setLhs) {
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (property instanceof Assign && property.context === 'object') {
            var {value} = property;
            var unwrappedValue = value.unwrapAll();
      ...
      astProperties(o) {
        ...
        return {
          ...
          properties: (function() {
            var ref1 = this.expandProperties();
            var results1 = [];
            for (var j = 0, len1 = ref1.length; j < len1; j++) {
              property = ref1[j];
  ...
  exports.ObjectProperty = ObjectProperty = class ObjectProperty extends Base {
    ...
    astProperties(o) {
      var ref1, ref2;
      var isComputedPropertyName = (this.key instanceof Value && this.key.base instanceof ComputedPropertyName) || this.key.unwrap() instanceof StringWithInterpolations;
      var keyAst = this.key.ast(o, LEVEL_LIST);
      return {
  ...
  exports.Arr = Arr = (function() {
    class Arr extends Base {
      ...
      isAssignable(opts) {
        var allowEmptyArray, allowExpansion, allowNontrailingSplat;
      ...
      compileNode(o) {
        ...
        o.indent += TAB;
        var fragmentIsElision = function([fragment]) {
          return fragment.type === 'Elision' && fragment.code.trim() === ',';
        };
        // Detect if `Elision`s at the beginning of the array are processed (e.g. [, , , a]).
        var passedElision = false;
        var answer = [];
        var ref1 = this.objects;
        for (var j = 0, objIndex = j, len1 = ref1.length; j < len1; objIndex = ++j) {
          var obj = ref1[objIndex];
          var unwrappedObj = obj.unwrapAll();
        ...
        var compiledObjs = (function() {
          var ref2 = this.objects;
          var results1 = [];
          for (var k = 0, len2 = ref2.length; k < len2; k++) {
            obj = ref2[k];
        ...
        var includesLineCommentsOnNonFirstElement = false;
        for (var k = 0, index = k, len2 = compiledObjs.length; k < len2; index = ++k) {
          var fragments = compiledObjs[index];
          for (var l = 0, len3 = fragments.length; l < len3; l++) {
            var fragment = fragments[l];
        ...
        if (includesLineCommentsOnNonFirstElement || indexOf.call(fragmentsToText(answer), '\n') >= 0) {
          for (var p = 0, fragmentIndex = p, len4 = answer.length; p < len4; fragmentIndex = ++p) {
          ...
          for (var q = 0, len5 = answer.length; q < len5; q++) {
            fragment = answer[q];
            if (fragment.isHereComment) {
      ...
      propagateLhs(setLhs) {
        ...
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          var unwrappedObject = object.unwrapAll();
  ...
  exports.Class = Class = (function() {
    class Class extends Base {
      ...
      compileNode(o) {
        ...
        var executableBody = this.walkBody(o);
        if (this.parent instanceof Value && !this.parent.hasProperties()) {
          // Special handling to allow `class expr.A extends A` declarations
          var parentName = this.parent.base.value;
        }
      ...
      walkBody(o) {
        var assign, initializerExpression;
        this.ctor = null;
        this.boundMethods = [];
        var executableBody = null;
        var initializer = [];
        var {expressions} = this.body;
        var i = 0;
        var ref1 = expressions.slice();
        for (var j = 0, len1 = ref1.length; j < len1; j++) {
          var expression = ref1[j];
        ...
        for (var k = 0, len2 = initializer.length; k < len2; k++) {
          var method = initializer[k];
        ...
        if (initializer.length !== expressions.length) {
          this.body.expressions = (function() {
            var results1 = [];
            for (var l = 0, len3 = initializer.length; l < len3; l++) {
      ...
      addInitializerMethod(assign) {
        var method, operatorToken, variable;
        ({
          variable,
          value: method,
          operatorToken
        ...
        if (method.isStatic) {
          ...
          var methodName = variable.base;
          method.name = new (methodName.shouldCache() ? Index : Access)(methodName);
          method.name.updateLocationDataIfMissing(methodName.locationData);
          var isConstructor = methodName instanceof StringLiteral ? methodName.originalValue === 'constructor' : methodName.value === 'constructor';
          if (isConstructor) {
      ...
      addClassProperty(assign) {
        var {variable, value, operatorToken} = assign;
        var {staticClassName} = variable.looksStatic(this.name);
      ...
      makeDefaultConstructor() {
        var ctor = this.addInitializerMethod(new Assign(new Value(new PropertyName('constructor')), new Code()));
        this.body.unshift(ctor);
        if (this.parent) {
          ctor.body.push(new SuperCall(new Super(), [new Splat(new IdentifierLiteral('arguments'))]));
        }
        if (this.externalCtor) {
          var applyCtor = new Value(this.externalCtor, [new Access(new PropertyName('apply'))]);
          var applyArgs = [new ThisLiteral(), new IdentifierLiteral('arguments')];
      ...
      declareName(o) {
        ...
        var alreadyDeclared = o.scope.find(name.value);
  ...
  exports.ExecutableClassBody = ExecutableClassBody = (function() {
    class ExecutableClassBody extends Base {
      ...
      compileNode(o) {
        ...
        var params = [];
        var args = [new ThisLiteral()];
        var wrapper = new Code(params, this.body);
        var klass = new Parens(new Call(new Value(wrapper, [new Access(new PropertyName('call'))]), args));
        this.body.spaced = true;
        o.classScope = wrapper.makeScope(o.scope);
        this.name = (ref1 = this.class.name) != null ? ref1 : o.classScope.freeVariable(this.defaultClassVariableName);
        var ident = new IdentifierLiteral(this.name);
        var directives = this.walkBody();
        this.setContext();
        if (this.class.hasNameClash) {
          var parent = new IdentifierLiteral(o.classScope.freeVariable('superClass'));
          wrapper.params.push(new Param(parent));
          args.push(this.class.parent);
          this.class.parent = parent;
        }
        if (this.externalCtor) {
          var externalCtor = new IdentifierLiteral(o.classScope.freeVariable('ctor', {
            reserve: false
          }));
      ...
      addProperties(assigns) {
        var assign, base, name, prototype, value, variable;
        var result = (function() {
          var results1 = [];
          for (var j = 0, len1 = assigns.length; j < len1; j++) {
            assign = assigns[j];
      ...
      astAssertions(o) {
        ...
        if (((ref1 = this.assertions) != null ? ref1.properties : void 0) != null) {
          return this.assertions.properties.map((assertion) => {
            var {start, end, loc, left, right} = assertion.ast(o);
            return {
  ...
  exports.ExportDeclaration = ExportDeclaration = class ExportDeclaration extends ModuleDeclaration {
    compileNode(o) {
      ...
      var exportDefault = this instanceof ExportDefaultDeclaration;
      if (!exportDefault && (this.clause instanceof Assign || this.clause instanceof Class)) {
  ...
  exports.ExportNamedDeclaration = ExportNamedDeclaration = class ExportNamedDeclaration extends ExportDeclaration {
    astProperties(o) {
      ...
      var clauseAst = this.clause.ast(o);
    ...
    class ModuleSpecifierList extends Base {
      ...
      compileNode(o) {
        ...
        if (this.specifiers.length !== 0) {
          code.push(this.makeCode(`{\n${o.indent}`));
          for (var j = 0, index = j, len1 = compiledList.length; j < len1; index = ++j) {
  ...
  exports.ImportSpecifier = ImportSpecifier = class ImportSpecifier extends ModuleSpecifier {
    ...
    astProperties(o) {
      var ref1, ref2;
      var originalAst = this.original.ast(o);
      return {
    ...
    class Assign extends Base {
      ...
      addScopeVariables(o, {allowAssignmentToExpansion = false, allowAssignmentToNontrailingSplat = false, allowAssignmentToEmptyArray = false, allowAssignmentToComplexSplat = false} = {}) {
        ...
        return varBase.eachName((name) => {
          ...
          var message = isUnassignable(name.value);
          if (message) {
            name.error(message);
          }
          ...
          if (this.moduleDeclaration) {
            ...
            if (name.comments && !o.scope.hasComment(name.value) && !(this.value instanceof Class) && name.comments.every(function(comment) {
              return comment.here && !comment.multiline;
            })) {
              var commentsNode = new IdentifierLiteral(name.value);
              commentsNode.comments = name.comments;
              var commentFragments = [];
              this.compileCommentFragments(o, commentsNode, commentFragments);
              return o.scope.comment(name.value, commentFragments);
            }
          }
        });
      }
      ...
      makeDeclaration(o) {
        ...
        if (!(o.level === LEVEL_TOP && !o.undeclarable && this.variable.isDeclarable(o) && (this.context == null))) {
          return;
        }
        return this.eachName((name) => {
          if (o.scope.laterVar(name.value)) {
            return this.declaration = true; // at least one variable freshly declared
          }
        });
      }
      ...
      compileNode(o) {
        var name, properties, prototype, ref1, ref2, ref3, ref4;
        var isValue = this.variable instanceof Value;
        if (isValue) {
        ...
        this.makeDeclaration(o); // after compiling LHS, so has attachments
        ...
        if (this.declaration) {
          return [this.makeCode("var "), ...answer];
        ...
        } else if (o.level > LEVEL_LIST || isValue && this.variable.base instanceof Obj && !this.nestedLhs && !(this.param === true)) {
          return this.wrapInParentheses(answer);
        } else {
          return answer;
        }
      ...
      compileObjectDestruct(o) {
        ...
        var splatProp = splat.name;
        var assigns = [];
        var refVal = new Value(new IdentifierLiteral(o.scope.freeVariable('ref')));
      ...
      compileDestructuring(o) {
        var top = o.level === LEVEL_TOP;
        var {value} = this;
        var {objects} = this.variable.base;
        ...
        var {splats, expans, splatsAndExpans} = this.getAndCheckSplatsAndExpansions();
        var isSplat = (splats != null ? splats.length : void 0) > 0;
        var isExpans = (expans != null ? expans.length : void 0) > 0;
        var vvar = value.compileToFragments(o, LEVEL_LIST);
        var vvarText = fragmentsToText(vvar);
        var assigns = [];
        var pushAssign = (variable, val) => {
        ...
        if (isSplat) {
          var splatVar = objects[splats[0]].name.unwrap();
          if (splatVar instanceof Arr || splatVar instanceof Obj) {
            var splatVarRef = new IdentifierLiteral(o.scope.freeVariable('ref'));
            objects[splats[0]].name = splatVarRef;
            var splatVarAssign = function() {
        ...
        var objIsUnassignable = function(objs) {
          for (var j = 0, len1 = objs.length; j < len1; j++) {
            obj = objs[j];
            if (!obj.isAssignable()) {
              return true;
            }
        ...
        var loopObjects = (objs, vvar, vvarTxt) => {
          ...
          for (var j = 0, i = j, len1 = objs.length; j < len1; i = ++j) {
            ...
            if (obj instanceof Assign && obj.context === 'object') {
              ...
              var idx = vvar.this ? vvar.properties[0].name : new PropertyName(vvar.unwrap().value);
              var acc = idx.unwrap() instanceof PropertyName;
              var vval = new Value(value, [new (acc ? Access : Index)(idx)]);
            } else {
              // `obj` is [a...], {a...} or a
              vvar = (function() {
                switch (false) {
            ...
            var message = isUnassignable(vvar.unwrap().value);
            if (message) {
              vvar.error(message);
            }
        ...
        var assignObjects = (objs, vvar, vvarTxt) => {
          vvar = new Value(new Arr(objs, true));
          var vval = vvarTxt instanceof Value ? vvarTxt : new Value(new Literal(vvarTxt));
          return pushAssign(vvar, vval);
        };
        var processObjects = function(objs, vvar, vvarTxt) {
        ...
        if (splatsAndExpans.length) {
          var expIdx = splatsAndExpans[0];
          var leftObjs = objects.slice(0, expIdx + (isSplat ? 1 : 0));
          var rightObjs = objects.slice(expIdx + 1);
      ...
      getAndCheckSplatsAndExpansions() {
        ...
        var splats = (function() {
          var j, len1;
          var results1 = [];
          for (j = 0, i = j, len1 = objects.length; j < len1; i = ++j) {
            obj = objects[i];
        ...
        var splatsAndExpans = [...splats, ...expans];
      ...
      compileSplice(o) {
        ...
        var unwrappedVar = this.variable.unwrapAll();
        ...
        var answer = [].concat(this.makeCode(`${utility('splice', o)}.apply(${name}, [${fromDecl}, ${to}].concat(`), valDef, this.makeCode(")), "), valRef);
        if (o.level > LEVEL_TOP) {
          return this.wrapInParentheses(answer);
        } else {
          return answer;
      ...
      astNode(o) {
        ...
        if (this.isConditional()) {
          var variable = this.variable.unwrap();
  ...
  exports.Code = Code = (function() {
    class Code extends Base {
      ...
      // function body.
      compileNode(o) {
        var m, methodScope, ref1, ref2, ref3, ref5, ref6, ref7;
        ...
        var params = [];
        var exprs = [];
        var thisAssignments = (ref2 = (ref3 = this.thisAssignments) != null ? ref3.slice() : void 0) != null ? ref2 : [];
        var paramsAfterSplat = [];
        var haveSplatParam = false;
        var haveBodyParam = false;
        ...
        this.eachParamName(function(name, node, param, obj) {
          if (node.this) {
            ...
            var target = new IdentifierLiteral(o.scope.freeVariable(name, {
              reserve: false
            }));
            ...
            var replacement = param.name instanceof Obj && obj instanceof Assign && obj.operatorToken.value === '=' ? new Assign(new IdentifierLiteral(name), target, 'object') : target; //, operatorToken: new Literal ':'
            param.renameParam(node, replacement);
            return thisAssignments.push(new Assign(node, target));
          }
        });
        var ref4 = this.params;
        ...
        for (var j = 0, i = j, len1 = ref4.length; j < len1; i = ++j) {
          var param = ref4[i];
          ...
          if (param.splat || param instanceof Expansion) {
            ...
            if (param.splat) {
              if (param.name instanceof Arr || param.name instanceof Obj) {
                ...
                var splatParamName = o.scope.freeVariable('arg');
            ...
            if (param.shouldCache() || haveBodyParam) {
              ...
              if (param.value != null) {
                var condition = new Op('===', param, new UndefinedLiteral());
                var ifTrue = new Assign(new Value(param.name), param.value);
            ...
            if (!haveSplatParam) {
              ...
              if (param.name instanceof Arr || param.name instanceof Obj) {
                ...
                // is compiled.
                var paramToAddToScope = param.value != null ? param : ref;
        ...
        if (paramsAfterSplat.length !== 0) {
          ...
          exprs.unshift(new Assign(new Value(new Arr([
            ...
            ...((function() {
              var results1 = [];
              for (var k = 0, len2 = paramsAfterSplat.length; k < len2; k++) {
        ...
        if (this.isMethod && this.bound && !this.isStatic && this.classVariable) {
          var boundMethodCheck = new Value(new Literal(utility('boundMethodCheck', o)));
        ...
        if (this.bound && this.isGenerator) {
          var yieldNode = this.body.contains(function(node) {
        ...
        for (var k = 0, i = k, len2 = params.length; k < len2; i = ++k) {
          param = params[i];
          if (i !== 0) {
          ...
          var scopeVariablesCount = o.scope.variables.length;
          signature.push(...param.compileToFragments(o, LEVEL_PAREN));
          if (scopeVariablesCount !== o.scope.variables.length) {
            var generatedVariables = o.scope.spliceVariables(scopeVariablesCount);
        ...
        if (((ref7 = this.funcGlyph) != null ? ref7.comments : void 0) != null) {
          var ref8 = this.funcGlyph.comments;
          for (var l = 0, len3 = ref8.length; l < len3; l++) {
            var comment = ref8[l];
        ...
        if (!this.body.isEmpty()) {
          var body = this.body.compileWithDeclarations(o);
        }
        ...
        var answer = this.joinFragmentArrays((function() {
          var results1 = [];
          for (var p = 0, len4 = modifiers.length; p < len4; p++) {
            m = modifiers[p];
      ...
      checkSuperCallsInConstructorBody() {
        ...
        var seenSuper = this.eachSuperCall(this.body, (superCall) => {
          if (this.ctor === 'base') {
      ...
      expandCtorSuper(thisAssignments) {
        ...
        var haveThisParam = thisAssignments.length && thisAssignments.length !== ((ref1 = this.thisAssignments) != null ? ref1.length : void 0);
        if (this.ctor === 'derived' && !seenSuper && haveThisParam) {
          var param = thisAssignments[0].variable;
        ...
        context.traverseChildren(true, (child) => {
          if (child instanceof SuperCall) {
            ...
            if (!child.variable.accessor) {
              var childArgs = child.args.filter(function(arg) {
      ...
      astNode(o) {
        ...
        this.disallowLoneExpansionAndMultipleSplats();
        var seenSuper = this.checkSuperCallsInConstructorBody();
      ...
      astLocationData() {
        var functionLocationData = super.astLocationData();
        if (!this.isMethod) {
          return functionLocationData;
        }
        var astLocationData = mergeAstLocationData(this.name.astLocationData(), functionLocationData);
    ...
    class Param extends Base {
      constructor(name1, value1, splat1) {
        ...
        if (this.name instanceof Obj && this.name.generated) {
          var token = this.name.objects[0].operatorToken;
      ...
      eachName(iterator, name = this.name) {
        var ref1;
        var checkAssignabilityOfLiteral = function(literal) {
          var message = isUnassignable(literal.value);
          if (message) {
        ...
        var atParam = (obj, originalObj = null) => {
        ...
        var ref2 = (ref1 = name.objects) != null ? ref1 : [];
        for (var j = 0, len1 = ref2.length; j < len1; j++) {
          var obj = ref2[j];
          // Save original obj.
          var nObj = obj;
      ...
      renameParam(node, newNode) {
        var isNode = function(candidate) {
          return candidate === node;
        };
        var replacement = (node, parent) => {
  ...
  exports.Splat = Splat = (function() {
    class Splat extends Base {
      ...
      compileNode(o) {
        var compiledSplat = [this.makeCode('...'), ...this.name.compileToFragments(o, LEVEL_OP)];
        if (!this.jsx) {
  ...
  exports.Elision = Elision = (function() {
    class Elision extends Base {
      compileToFragments(o, level) {
        var fragment = super.compileToFragments(o, level);
    ...
    Elision.prototype.isDeclarable = YES;
  ...
  exports.While = While = (function() {
    class While extends Base {
      ...
      jumps() {
        ...
        for (var j = 0, len1 = expressions.length; j < len1; j++) {
          var node = expressions[j];
      ...
      compileNode(o) {
        ...
        var answer = [].concat(this.makeCode(set + this.tab + "while ("), this.processedCondition().compileToFragments(o, LEVEL_PAREN), this.makeCode(") {"), body, this.makeCode("}"));
        if (this.returns) {
  ...
  exports.Op = Op = (function() {
    class Op extends Base {
      ...
      compileNode(o) {
        if (this.isInOperator()) {
          var inNode = new In(this.first, this.second);
      ...
      compileExistence(o, checkOnlyUndefined) {
        if (this.first.shouldCache()) {
          var ref = new IdentifierLiteral(o.scope.freeVariable('ref'));
          var fst = new Parens(new Assign(ref, this.first));
        } else {
          fst = this.first;
          ref = fst;
        }
      ...
      compileFloorDivision(o) {
        ...
        var second = this.second.shouldCache() ? new Parens(this.second) : this.second;
      ...
      chainAstProperties(o) {
        var operand;
        var operators = [this.operatorAst()];
        var operands = [this.second];
        var currentOp = this.first;
        while (true) {
        ...
        return {
          ...
          operands: (function() {
            var results1 = [];
            for (var j = 0, len1 = operands.length; j < len1; j++) {
              operand = operands[j];
      ...
      astProperties(o) {
        ...
        var firstAst = this.first.ast(o, LEVEL_OP);
        var secondAst = (ref1 = this.second) != null ? ref1.ast(o, LEVEL_OP) : void 0;
        var operatorAst = this.operatorAst();
        switch (false) {
          case !this.isUnary():
            var argument = this.isYield() && this.first.unwrap().value === '' ? null : firstAst;
            if (this.isAwait()) {
              return {argument};
            }
            if (this.isYield()) {
    ...
    class In extends Base {
      ...
      compileNode(o) {
        if (this.array instanceof Value && this.array.isArray() && this.array.base.objects.length) {
          var ref1 = this.array.base.objects;
          for (var j = 0, len1 = ref1.length; j < len1; j++) {
            var obj = ref1[j];
            if (!(obj instanceof Splat)) {
              continue;
  ...
  exports.Try = Try = (function() {
    class Try extends Base {
      ...
      compileNode(o) {
        ...
        var originalIndent = o.indent;
        o.indent += TAB;
        var tryPart = this.attempt.compileToFragments(o, LEVEL_TOP);
        var catchPart = this.catch ? this.catch.compileToFragments(merge(o, {
  ...
  exports.Catch = Catch = (function() {
    class Catch extends Base {
      ...
      makeReturn(results, mark) {
        var ret = this.recovery.makeReturn(results, mark);
        if (mark) {
          return;
        }
        this.recovery = ret;
      ...
      compileNode(o) {
        o.indent += TAB;
        var generatedErrorVariableName = o.scope.freeVariable('error', {
          reserve: false
        });
      ...
      checkUnassignable() {
        if (this.errorVariable) {
          var message = isUnassignable(this.errorVariable.unwrapAll().value);
          if (message) {
  ...
  exports.Throw = Throw = (function() {
    class Throw extends Base {
      ...
      compileNode(o) {
        var fragments = this.expression.compileToFragments(o, LEVEL_LIST);
  ...
  exports.Parens = Parens = (function() {
    class Parens extends Base {
      ...
      compileNode(o) {
        ...
        var shouldWrapComment = (ref1 = expr.comments) != null ? ref1.some(function(comment) {
        ...
        var bare = o.level < LEVEL_OP && !shouldWrapComment && (expr instanceof Op && !expr.isInOperator() || expr.unwrap() instanceof Call || (expr instanceof For && expr.returns)) && (o.level < LEVEL_COND || fragments.length <= 3);
  ...
  exports.StringWithInterpolations = StringWithInterpolations = (function() {
    class StringWithInterpolations extends Base {
      ...
      static fromStringLiteral(stringLiteral) {
        var updatedString = stringLiteral.withoutQuotesInLocationData();
        var updatedStringValue = new Value(updatedString).withLocationDataFrom(updatedString);
        ...
        expr.traverseChildren(false, (node) => {
          ...
          if (node instanceof StringLiteral) {
            ...
            if (salvagedComments.length !== 0) {
              for (var j = 0, len1 = salvagedComments.length; j < len1; j++) {
                var comment = salvagedComments[j];
            ...
            if ((unwrapped = (ref1 = node.expression) != null ? ref1.unwrapAll() : void 0) instanceof PassthroughLiteral && unwrapped.generated && !(isJsx && o.compiling)) {
              if (o.compiling) {
                var commentPlaceholder = new StringLiteral('').withLocationDataFrom(node);
                ...
                var empty = new Interpolation().withLocationDataFrom(node);
            ...
            if (elements.length !== 0 && !(elements[elements.length - 1] instanceof StringLiteral)) {
              var ref3 = node.comments;
              for (var k = 0, len2 = ref3.length; k < len2; k++) {
                comment = ref3[k];
                comment.unshift = false;
                comment.newLine = true;
              }
      ...
      compileNode(o) {
        ...
        if (this.jsxAttribute) {
          var wrapped = new Parens(new StringWithInterpolations(this.body));
          wrapped.jsxAttribute = true;
          return wrapped.compileNode(o);
        }
        var elements = this.extractElements(o, {
          isJsx: this.jsx
        });
        var fragments = [];
        if (!this.jsx) {
        ...
        for (var j = 0, len1 = elements.length; j < len1; j++) {
          ...
          if (element instanceof StringLiteral) {
            var unquotedElementValue = this.jsx ? element.unquotedValueForJSX : element.unquotedValueForTemplateLiteral;
            fragments.push(this.makeCode(unquotedElementValue));
          } else {
            if (!this.jsx) {
              fragments.push(this.makeCode('$'));
            }
            var code = element.compileToFragments(o, LEVEL_PAREN);
      ...
      isNestedTag(element) {
        var call = typeof element.unwrapAll === "function" ? element.unwrapAll() : void 0;
      ...
      astProperties(o) {
        ...
        var quasis = [];
        var expressions = [];
        for (var j = 0, index = j, len1 = elements.length; j < len1; index = ++j) {
          ...
          if (element instanceof StringLiteral) {
            ...
            var node = expression == null ? (emptyInterpolation = new EmptyInterpolation(), emptyInterpolation.locationData = emptyExpressionLocationData({
              interpolationNode: element,
              openingBrace: '#{',
              closingBrace: '}'
  ...
  exports.For = For = (function() {
    class For extends While {
      ...
      addSource(source) {
        var base1, ref1, ref2, ref3;
        ({source: this.source = false} = source);
        var attribs = ["name", "index", "guard", "step", "own", "ownTag", "await", "awaitTag", "object", "from"];
        for (var j = 0, len1 = attribs.length; j < len1; j++) {
          var attr = attribs[j];
        ...
        this.returns = false;
        var ref4 = ['source', 'guard', 'step', 'name', 'index'];
        ...
        for (var k = 0, len2 = ref4.length; k < len2; k++) {
          var attribute = ref4[k];
          if (!this[attribute]) {
            continue;
          }
          this[attribute].traverseChildren(true, (node) => {
            if (node.comments) {
              var ref5 = node.comments;
              for (var l = 0, len3 = ref5.length; l < len3; l++) {
                var comment = ref5[l];
      ...
      compileNode(o) {
        ...
        var scope = o.scope;
        if (!this.pattern) {
          var name = this.name && (this.name.compile(o, LEVEL_LIST));
        }
        ...
        if (this.returns) {
          var rvar = scope.freeVariable('results', {
            laterVar: true
          });
        }
        if (this.from) {
          if (this.pattern) {
            var ivar = scope.freeVariable('x', {
              single: true
            });
          }
        } else {
        ...
        if (kvar !== ivar) {
          var kvarDeclare = `, ${kvar} = ${ivar}`;
          var kvarAssign = `${kvar} = `;
        } else {
          kvarDeclare = kvarAssign = '';
        }
        if (this.step && !this.range) {
          var [step, stepVar] = this.cacheToCodeFragments(this.step.cache(o, LEVEL_LIST, shouldCacheOrIsAssignable));
          if (this.step.isNumber()) {
            var stepNum = parseNumber(stepVar);
          }
        }
        if (this.pattern) {
          name = ivar;
        ...
        if (this.range) {
          var forPartFragments = source.compileToFragments(merge(o, {
            index: ivar,
            name,
            step: this.step,
          ...
          var svar = this.source.compile(o, LEVEL_LIST);
          if ((name || this.own) && !(this.source.unwrap() instanceof IdentifierLiteral)) {
            var ref = scope.freeVariable('ref', {
              laterVar: true
            });
            defPart += `${this.tab}var ${ref} = ${svar};\n`;
            svar = ref;
          }
          var canDeclare = (!this.step || (stepNum != null)) && ((this.index == null) || this.index.isDeclarable(o));
          if (name && !this.pattern && !this.from) {
            var namePart = `${name} = ${svar}[${kvar}]`;
            if (this.name.isDeclarable(o) && o.scope.laterVar(name)) {
              namePart = `var ${namePart}`;
            }
          }
          if (!this.object && !this.from) {
            ...
            var down = stepNum < 0;
            if (canDeclare) {
              var ivarLater = o.scope.laterVar(ivar);
              var kvarLater = o.scope.laterVar(kvar);
            }
            if (!(this.step && (stepNum != null) && down)) {
              var lvar = scope.freeVariable('len', {
                laterVar: canDeclare
              });
              var declare = `${ivar} = 0${kvarDeclare}, ${lvar} = ${svar}.length`;
              if (canDeclare) { // declare lvar at least
                declare = `var ${declare}`;
              }
            }
            var declareDown = `${ivar} = ${svar}.length - 1${kvarDeclare}`;
            if (ivarLater || kvarLater) {
              declareDown = `var ${declareDown}`;
            }
        ...
        if (this.returns) {
          var resultPart = `${this.tab}var ${rvar} = [];\n`;
          var returnResult = `\n${this.tab}return ${rvar};`;
        ...
        if (this.object) {
          var forPart = `${kvar} in ${svar}`;
          if (o.scope.laterVar(kvar)) {
            forPart = `var ${forPart}`;
          }
          forPartFragments = [this.makeCode(forPart)];
          if (this.own) {
        ...
        var fragments = [this.makeCode(defPart)];
        if (resultPart) {
          fragments.push(this.makeCode(resultPart));
        }
        var forCode = this.await ? 'for ' : 'for (';
        var forClose = this.await ? '' : ')';
  ...
  exports.Switch = Switch = (function() {
    class Switch extends Base {
      ...
      compileNode(o) {
        ...
        var ref1 = this.cases;
        for (var j = 0, i = j, len1 = ref1.length; j < len1; i = ++j) {
          var {conditions, block} = ref1[i];
          var ref2 = flatten([conditions]);
          for (var k = 0, len2 = ref2.length; k < len2; k++) {
            var cond = ref2[k];
            if (!this.subject) {
              cond = cond.invert();
          ...
          var expr = this.lastNode(block.expressions);
      ...
      casesAst(o) {
        ...
        var cases = [];
        var ref1 = this.cases;
        for (var j = 0, caseIndex = j, len1 = ref1.length; j < len1; caseIndex = ++j) {
          var kase = ref1[caseIndex];
          ({
            conditions: tests,
            block: consequent
          } = kase);
          var tests = flatten([tests]);
          var lastTestIndex = tests.length - 1;
          for (var k = 0, testIndex = k, len2 = tests.length; k < len2; testIndex = ++k) {
            var test = tests[testIndex];
            var testConsequent = testIndex === lastTestIndex ? consequent : null;
            var caseLocationData = test.locationData;
        ...
        var results1 = [];
        for (var l = 0, len3 = cases.length; l < len3; l++) {
          kase = cases[l];
  ...
  exports.If = If = (function() {
    class If extends Base {
      ...
      compileStatement(o) {
        ...
        var cond = this.processedCondition().compileToFragments(o, LEVEL_PAREN);
        var body = this.ensureBlock(this.body).compileToFragments(merge(o, {indent}));
      ...
      compileExpression(o) {
        var cond = this.processedCondition().compileToFragments(o, LEVEL_COND);
      ...
      astProperties(o) {
        var ref1, ref2, ref3, ref4;
        var isStatement = this.isStatementAst(o);
        return {
  ...
  var HEREGEX_OMIT = /((?:\\\\)+)|\\(\s)|\s+(?:#.*)?/g; // Consume (and preserve) an even number of backslashes.
  ...
  var multident = function(code, tab, includingFirstLine = true) {
    var endsWithNewLine = code[code.length - 1] === '\n';
  ...
  var shouldCacheOrIsAssignable = function(node) {
  ...
  var makeDelimitedLiteral = function(body, {
    ...
    var escapeTemplateLiteralCurlies = delimiterOption === '`';
    var regex = RegExp(`(\\\\\\\\)|(\\\\0(?=\\d))${convertTrailingNullEscapes ? /|(\\0)$/.source : '' // Escaped backslash. // Trailing null character that could be mistaken as octal escape.
    ...
    body = body.replace(regex, function(match, backslash, nul, ...args) {
      var trailingNullEscape = convertTrailingNullEscapes ? args.shift() : void 0;
      var delimiter = escapeDelimiter ? args.shift() : void 0;
      var templateLiteralCurly = escapeTemplateLiteralCurlies ? args.shift() : void 0;
      var lf = escapeNewlines ? args.shift() : void 0;
      var [cr, ls, ps, other] = args;
      switch (false) {
    ...
    var printedDelimiter = includeDelimiters ? delimiterOption : '';
    return `${printedDelimiter}${body}${printedDelimiter}`;
  };

  var sniffDirectives = function(expressions, {notFinalExpression} = {}) {
    var results1, unwrapped;
    var index = 0;
  ...
  var astAsBlockIfNeeded = function(node, o) {
    var unwrapped = node.unwrap();
  ...
  var isAstLocGreater = function(a, b) {
    if (a.line > b.line) {
      return true;
    }
    if (a.line !== b.line) {
  ...
  var isLocationDataStartGreater = function(a, b) {
  ...
  var isLocationDataEndGreater = function(a, b) {
  ...
  var zeroWidthLocationDataFromEndLocation = function({
      range: [, endRange],
      last_line_exclusive,
  ...
  var extractSameLineLocationDataFirst = function(numChars) {
    return function({
        range: [startRange],
        first_line,
        first_column
  ...
  var extractSameLineLocationDataLast = function(numChars) {
    return function({
        range: [, endRange],
        last_line,
        last_column,
  ...
  var emptyExpressionLocationData = function({
      interpolationNode: element,
      openingBrace,
      closingBrace
    }) {
