...
(function() {
  ...
  var {merge, updateSyntaxError} = require('./helpers');
  ...
  var replDefaults = {
    ...
    historyFile: (function() {
      var historyPath = process.env.XDG_CACHE_HOME || process.env.HOME;
      if (historyPath) {
    ...
    eval: function(input, context, filename, cb) {
      ...
      var {Block, Assign, Value, Literal, Call, Code, Root} = require('./nodes');
      try {
        ...
        var ast = CoffeeScript.nodes(tokens).body;
        ...
        var result = runInContext(js, context, filename);
  ...
  var addMultilineHandler = function(repl) {
    var ref;
    var {inputStream, outputStream} = repl;
    // Node 0.11.12 changed API, prompt is now _prompt.
    var origPrompt = (ref = repl._prompt) != null ? ref : repl.prompt;
    var multiline = {
      enabled: false,
    ...
    var nodeLineListener = repl.listeners('line')[0];
  ...
  module.exports = {
    start: function(opts = {}) {
      var [major, minor, build] = process.versions.node.split('.').map(function(n) {
