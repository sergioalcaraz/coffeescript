...
(function() {
  ...
  var INVERSES, Rewriter, UNFINISHED,
    hasProp = {}.hasOwnProperty,
    indexOf = [].indexOf;

  var {throwSyntaxError, extractAllCommentTokens} = require('./helpers');

  // Move attached comments from one token to another.
  var moveComments = function(fromToken, toToken) {
    if (!fromToken.comments) {
      return;
    }
    if (toToken.comments && toToken.comments.length !== 0) {
      var unshiftedComments = [];
      var ref = fromToken.comments;
  ...
  var generate = function(tag, value, origin, commentsToken) {
    var token = [tag, value];
  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      rewrite(tokens1) {
        ...
        if (typeof process !== "undefined" && process !== null ? (ref = process.env) != null ? ref.DEBUG_TOKEN_STREAM : void 0 : void 0) {
          ...
          console.log(((function() {
            var ref1 = this.tokens;
            var results = [];
            for (var k = 0, len = ref1.length; k < len; k++) {
              t = ref1[k];
        ...
        if (typeof process !== "undefined" && process !== null ? (ref1 = process.env) != null ? ref1.DEBUG_REWRITTEN_TOKEN_STREAM : void 0 : void 0) {
          ...
          console.log(((function() {
            var ref2 = this.tokens;
            var results = [];
            for (var k = 0, len = ref2.length; k < len; k++) {
              t = ref2[k];
              results.push(t[0] + '/' + t[1] + (t.comments ? '*' : ''));
            }
            return results;
      ...
      removeLeadingNewlines() {
        var ref = this.tokens;
        for (var k = 0, i = k, len = ref.length; k < len; i = ++k) {
          var [tag] = ref[i];
          if (tag !== 'TERMINATOR') {
        ...
        var ref1 = this.tokens.slice(0, i);
        ...
        for (var l = 0, len1 = ref1.length; l < len1; l++) {
          var leadingNewlineToken = ref1[l];
      ...
      indexOfTag(i, ...pattern) {
        var ref1;
        var fuzz = 0;
        for (var k = 0, j = k, ref = pattern.length; (0 <= ref ? k < ref : k > ref); j = 0 <= ref ? ++k : --k) {
          if (pattern[j] == null) {
            continue;
          }
      ...
      addImplicitBracesAndParens() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          var implicitObjectIndent, nextToken, prevToken, ref, ref1, ref2, ref3, ref4, ref5, startIndex, startTag;
          var [tag] = token;
          var [prevTag] = prevToken = i > 0 ? tokens[i - 1] : [];
          var [nextTag] = nextToken = i < tokens.length - 1 ? tokens[i + 1] : [];
          ...
          // Helper functions
          var isImplicit = function(stackItem) {
            var ref;
            return stackItem != null ? (ref = stackItem[2]) != null ? ref.ours : void 0 : void 0;
          };
          var isImplicitObject = function(stackItem) {
            return isImplicit(stackItem) && (stackItem != null ? stackItem[0] : void 0) === '{';
          };
          var isImplicitCall = function(stackItem) {
            return isImplicit(stackItem) && (stackItem != null ? stackItem[0] : void 0) === '(';
          };
          var inImplicit = function() {
            return isImplicit(stackTop());
          };
          var inImplicitCall = function() {
            return isImplicitCall(stackTop());
          };
          var inImplicitObject = function() {
            return isImplicitObject(stackTop());
          };
          // Unclosed control statement inside implicit parens (like
          // class declaration or if-conditionals).
          var inImplicitControl = function() {
            var ref;
            return inImplicit() && ((ref = stackTop()) != null ? ref[0] : void 0) === 'CONTROL';
          };
          var startImplicitCall = function(idx) {
            stack.push([
              '(',
              idx,
              {
          ...
          var endImplicitCall = function() {
            stack.pop();
            tokens.splice(i, 0, generate('CALL_END', ')', ['', 'end of input', token[2]], prevToken));
            return i += 1;
          };
          var startImplicitObject = function(idx, {startsLine = true, continuationLineIndent} = {}) {
            stack.push([
              '{',
              idx,
              {
          ...
          var endImplicitObject = function(j) {
            j = j != null ? j : i;
            stack.pop();
            tokens.splice(j, 0, generate('}', '}', token, prevToken));
            return i += 1;
          };
          var implicitObjectContinues = (j) => {
            var nextTerminatorIdx = null;
          ...
          var inControlFlow = () => {
            var seenFor = this.findTagsBackwards(i, ['FOR']) && this.findTagsBackwards(i, ['FORIN', 'FOROF', 'FORFROM']);
            var controlFlow = seenFor || this.findTagsBackwards(i, ['WHILE', 'UNTIL', 'LOOP', 'LEADING_WHEN']);
            if (!controlFlow) {
              return false;
            }
            var isFunc = false;
            var tagCurrentLine = token[2].first_line;
          ...
          if (tag === ':') {
            ...
            var startsLine = s <= 0 || (ref3 = this.tag(s - 1), indexOf.call(LINEBREAKS, ref3) >= 0) || tokens[s - 1].newLine;
            // Are we just continuing an already declared object?
            // Including the case where we indent on the line after an explicit '{'.
            if (stackTop()) {
              var [stackTag, stackIdx] = stackTop();
              var stackNext = stack[stack.length - 2];
            ...
            var preObjectToken = i > 1 ? tokens[i - 2] : [];
          ...
          if (tag === 'TERMINATOR' && token.endsContinuationLineIndentation) {
            var {preContinuationLineIndent} = token.endsContinuationLineIndentation;
          ...
          if (indexOf.call(IMPLICIT_END, tag) >= 0 || (indexOf.call(CALL_CLOSERS, tag) >= 0 && newLine) || ((tag === '..' || tag === '...') && this.findTagsBackwards(i, ["INDEX_START"]))) {
            while (inImplicit()) {
              var [stackTag, stackIdx, {sameLine, startsLine}] = stackTop();
      ...
      rescueStowawayComments() {
        ...
        var shiftCommentsForward = function(token, i, tokens) {
          var ref, ref1;
        ...
        var shiftCommentsBackward = function(token, i, tokens) {
          var ref, ref1;
      ...
      addLocationDataToGeneratedTokens() {
        return this.scanTokens(function(token, i, tokens) {
          var column, nextLocation, prevLocation, ref, ref1;
          if (token[2]) {
            return 1;
          }
      ...
      fixIndentationLocationData() {
        ...
        var findPrecedingComment = (token, {afterPosition, indentSize, first, indented}) => {
          var tokenStart = token[2].range[0];
          var matches = function(comment) {
          ...
          if (first) {
            var lastMatching = null;
            var ref = this.allComments;
            for (var k = ref.length - 1; k >= 0; k += -1) {
              var comment = ref[k];
              if (matches(comment)) {
          ...
          for (var l = ref1.length - 1; l >= 0; l += -1) {
            comment = ref1[l];
            if (matches(comment)) {
              return comment;
            }
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          var isIndent = token[0] === 'INDENT';
          var prevToken = (ref2 = token.prevToken) != null ? ref2 : tokens[i - 1];
          var prevLocationData = prevToken[2];
          ...
          var useNextToken = token.explicit || token.generated;
          if (useNextToken) {
            var nextToken = token;
          ...
          var precedingComment = findPrecedingComment(useNextToken ? nextToken : token, {
      ...
      normalizeLines() {
        ...
        var leading_switch_when = null;
        var leading_if_then = null;
        // Count `THEN` tags
        var ifThens = [];
        var condition = function(token, i) {
        ...
        var closeElseTag = (tokens, i) => {
          var tlen = ifThens.length;
          if (!(tlen > 0)) {
            return i;
          }
          var lastThen = ifThens.pop();
          var [, outdentElse] = this.indentation(tokens[lastThen]);
        ...
        return this.scanTokens(function(token, i, tokens) {
          var ref, ref1, ref2;
          var [tag] = token;
          var conditionTag = (tag === '->' || tag === '=>') && this.findTagsBackwards(i, ['IF', 'WHILE', 'FOR', 'UNTIL', 'SWITCH', 'WHEN', 'LEADING_WHEN', '[', 'INDEX_START']) && !(this.findTagsBackwards(i, ['THEN', '..', '...']));
          if (tag === 'TERMINATOR') {
      ...
      exposeTokenDataToGrammar() {
        return this.scanTokens(function(token, i) {
          ...
          if (token.generated || (token.data && Object.keys(token.data).length !== 0)) {
            ...
            var ref1 = (ref = token.data) != null ? ref : {};
            for (var key in ref1) {
  ...
  var BALANCED_PAIRS = [['(', ')'], ['[', ']'], ['{', '}'], ['INDENT', 'OUTDENT'], ['CALL_START', 'CALL_END'], ['PARAM_START', 'PARAM_END'], ['INDEX_START', 'INDEX_END'], ['STRING_START', 'STRING_END'], ['INTERPOLATION_START', 'INTERPOLATION_END'], ['REGEX_START', 'REGEX_END']];
  ...
  for (var k = 0, len = BALANCED_PAIRS.length; k < len; k++) {
    var [left, right] = BALANCED_PAIRS[k];
  ...
  var IMPLICIT_CALL = ['IDENTIFIER', 'JSX_TAG', 'PROPERTY', 'NUMBER', 'INFINITY', 'NAN', 'STRING', 'STRING_START', 'REGEX', 'REGEX_START', 'JS', 'NEW', 'PARAM_START', 'CLASS', 'IF', 'TRY', 'SWITCH', 'THIS', 'DYNAMIC_IMPORT', 'IMPORT_META', 'NEW_TARGET', 'UNDEFINED', 'NULL', 'BOOL', 'UNARY', 'DO', 'DO_IIFE', 'YIELD', 'AWAIT', 'UNARY_MATH', 'SUPER', 'THROW', '@', '->', '=>', '[', '(', '{', '--', '++'];
