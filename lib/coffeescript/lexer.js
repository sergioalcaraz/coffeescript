...
(function() {
  ...
  var {Rewriter, INVERSES, UNFINISHED} = require('./rewriter');

  // Import the helpers we need.
  var {count, starts, compact, repeat, invertLiterate, merge, attachCommentsToNode, locationDataToString, throwSyntaxError, replaceUnicodeCodePointEscapes, flatten, parseNumber} = require('./helpers');
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    tokenize(code, opts = {}) {
      ...
      while (this.chunk = code.slice(i)) {
        var consumed = this.identifierToken() || this.commentToken() || this.whitespaceToken() || this.lineToken() || this.stringToken() || this.numberToken() || this.jsxToken() || this.regexToken() || this.jsToken() || this.literalToken();
        // Update position.
    ...
    identifierToken() {
      var match, ref, ref1, ref10, ref11, ref12, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, regExSuper;
      var inJSXTag = this.atJSXTag();
      var regex = inJSXTag ? JSX_ATTRIBUTE : IDENTIFIER;
      if (!(match = regex.exec(this.chunk))) {
        return 0;
      }
      var [input, id, colon] = match;
      ...
      if (id === 'do' && (regExSuper = /^(\s*super)(?!\(\))/.exec(this.chunk.slice(3)))) {
        ...
        this.token('CALL_END', ')');
        var [input, sup] = regExSuper;
        return sup.length + 3;
      }
      prev = this.prev();
      var tag = colon || (prev != null) && (((ref5 = prev[0]) === '.' || ref5 === '?.' || ref5 === '::' || ref5 === '?::') || !prev.spaced && prev[0] === '@') ? 'PROPERTY' : 'IDENTIFIER';
      var tokenData = {};
      ...
      // `get` or `set`, i.e. `get({foo: function () {}})`.
        ...
        if (prev.spaced && (ref9 = prev[0], indexOf.call(CALLABLE, ref9) >= 0) && /^[gs]et$/.test(prev[1]) && this.tokens.length > 1 && ((ref10 = this.tokens[this.tokens.length - 2][0]) !== '.' && ref10 !== '?.' && ref10 !== '@')) {
          ...
          var prevprev = this.tokens[this.tokens.length - 2];
      ...
      var tagToken = this.token(tag, id, {
        length: idLength,
        data: tokenData
      });
      if (alias) {
      ...
      if (colon) {
        var colonOffset = input.lastIndexOf(inJSXTag ? '=' : ':');
        var colonToken = this.token(':', ':', {
          offset: colonOffset
        });
    ...
    numberToken() {
      ...
      var parsedValue = parseNumber(number);
      var tokenData = {parsedValue};
      var tag = parsedValue === 2e308 ? 'INFINITY' : 'NUMBER';
      if (tag === 'INFINITY') {
    ...
    // interpolation.
    stringToken() {
      var end, i, match, ref, token, tokens;
      var [quote] = STRING_START.exec(this.chunk) || [];
      if (!quote) {
        return 0;
      }
      ...
      if (heredoc) {
        ...
        var doc = ((function() {
          var j, len;
          var results = [];
          for (j = 0, i = j, len = tokens.length; j < len; i = ++j) {
            token = tokens[i];
    ...
    commentToken(chunk = this.chunk, {heregex, returnCommentTokens = false, offsetInChunk = 0} = {}) {
      var commentAttachment, i, indentSize, leadingNewlineOffset, leadingWhitespace, length, match, noIndent, nonInitial, precededByBlankLine;
      if (!(match = chunk.match(COMMENT))) {
        return 0;
      }
      var [commentWithSurroundingWhitespace, hereLeadingWhitespace, hereComment, hereTrailingWhitespace, lineComment] = match;
      var contents = null;
      // Does this comment follow code on the same line?
      var leadingNewline = /^\s*\n+\s*#/.test(commentWithSurroundingWhitespace);
      if (hereComment) {
        var matchIllegal = HERECOMMENT_ILLEGAL.exec(hereComment);
        if (matchIllegal) {
        ...
        var precedingNonCommentLines = '';
        var hasSeenFirstCommentLine = false;
      ...
      var getIndentSize = function({leadingWhitespace, nonInitial}) {
        var lastNewlineIndex = leadingWhitespace.lastIndexOf('\n');
      ...
      var commentAttachments = (function() {
        var j, len;
        var results = [];
        for (j = 0, i = j, len = contents.length; j < len; i = ++j) {
      ...
      if (!prev) {
        ...
        var placeholderToken = this.makeToken('JS', '', {
    ...
    regexToken() {
      var commentIndex, commentOpts, match, matchedComment, ref, ref1;
      switch (false) {
      ...
      var [flags] = REGEX_FLAGS.exec(this.chunk.slice(index));
      var end = index + flags.length;
      var origin = this.makeToken('REGEX', null, {
        length: end
      });
      switch (false) {
    ...
    lineToken({chunk = this.chunk, offset = 0} = {}) {
      ...
      var indent = match[0];
      var prev = this.prev();
      var backslash = (prev != null ? prev[0] : void 0) === '\\';
      ...
      var newIndentLiteral = size > 0 ? indent.slice(-size) : '';
      ...
      var minLiteralLength = Math.min(newIndentLiteral.length, this.indentLiteral.length);
      ...
      if (size > this.indent) {
        ...
        var endsContinuationLineIndentation = this.continuationLineAdditionalIndent > 0;
    ...
    outdentToken({moveOut, noNewlines, outdentLength = 0, offset = 0, indentSize, endsContinuationLineIndentation}) {
      ...
      while (moveOut > 0) {
        ...
        if (!lastIndent) {
          ...
          var dent = this.indents.pop() + this.outdebt;
      ...
      if (!(this.tag() === 'TERMINATOR' || noNewlines)) {
        var terminatorToken = this.token('TERMINATOR', '\n', {
    ...
    jsxToken() {
      var end, jsxTag, openingTagToken, prev, ref, tokens;
      var firstChar = this.chunk[0];
      // Check the previous token to detect if attribute is spread.
      var prevChar = this.tokens.length > 0 ? this.tokens[this.tokens.length - 1][0] : '';
      if (firstChar === '<') {
        var match = JSX_IDENTIFIER.exec(this.chunk.slice(1)) || JSX_FRAGMENT_IDENTIFIER.exec(this.chunk.slice(1));
        ...
        var tagToken = this.token('JSX_TAG', id, {
          length: id.length + 1,
          data: {
        ...
        if (this.chunk.slice(0, 2) === '/>') { // Self-closing tag.
          ...
          var [, fullTagName] = match;
          var afterTag = end + fullTagName.length;
          ...
          var endToken = this.token('CALL_END', ')', {
            offset: end - 2,
    ...
    literalToken() {
      var match, ref, ref1, ref2, ref3, ref4, ref5;
      ...
      if (prev && indexOf.call(['=', ...COMPOUND_ASSIGN], value) >= 0) {
        ...
        if (prev && prev[0] !== 'PROPERTY') {
          var origin = (ref2 = prev.origin) != null ? ref2 : prev;
    ...
    tagDoIife(tokenIndex) {
      var tok = this.tokens[tokenIndex != null ? tokenIndex : this.tokens.length - 1];
    ...
    matchWithInterpolations(regex, delimiter, closingDelimiter = delimiter, interpolators = /^#\{/) {
      ...
      while (true) {
        ...
        var [interpolator] = match;
        // To remove the `#` in `#{`.
        var interpolationOffset = interpolator.length - 1;
        var [line, column, offset] = this.getLineAndColumnFromChunk(offsetInChunk + interpolationOffset);
        var rest = str.slice(interpolationOffset);
        ({
          tokens: nested,
          index
        ...
        var braceInterpolator = str[index - 1] === '}';
        ...
        if (!braceInterpolator) {
          ...
          var open = this.makeToken('INTERPOLATION_START', '(', {
            offset: offsetInChunk,
            length: 0,
            generated: true
          });
          var close = this.makeToken('INTERPOLATION_END', ')', {
            offset: offsetInChunk + index,
            length: 0,
            generated: true
          });
          var nested = [open, ...nested, close];
        }
    ...
    mergeInterpolationTokens(tokens, options, fn) {
      var lastToken, ref, ref1, rparen;
      var {quote, indent, double, heregex, endOffset, jsx} = options;
      if (tokens.length > 1) {
        var lparen = this.token('STRING_START', '(', {
      ...
      var $ = tokens.length - 1;
      for (var j = 0, i = j, len = tokens.length; j < len; i = ++j) {
        var token = tokens[i];
        var [tag, value] = token;
        switch (tag) {
          case 'TOKENS':
            ...
            // sane location data.
            var locationToken = value[0];
            var tokensToPush = value;
            break;
          case 'NEOSTRING':
    ...
    getLocationDataCompensation(start, end) {
      ...
      while (current <= end) {
        ...
        var compensation = this.locationDataCompensations[current];
    ...
    getLineAndColumnFromChunk(offset) {
      var lastLine, ref;
      var compensation = this.getLocationDataCompensation(this.chunkOffset, this.chunkOffset + offset);
      if (offset === 0) {
        return [this.chunkLine, this.chunkColumn + compensation, this.chunkOffset + compensation];
      }
      if (offset >= this.chunk.length) {
        var string = this.chunk;
      } else {
      ...
      if (lineCount > 0) {
        ...
        var previousLinesCompensation = this.getLocationDataCompensation(this.chunkOffset, this.chunkOffset + offset - column);
        if (previousLinesCompensation < 0) {
          // Don't recompensate for initially inserted newline.
          previousLinesCompensation = 0;
        }
        var columnCompensation = this.getLocationDataCompensation(this.chunkOffset + offset + previousLinesCompensation - column, this.chunkOffset + offset + previousLinesCompensation);
      } else {
    ...
    makeToken(tag, value, {
      ...
      var token = [tag, value, this.makeLocationData({offsetInChunk, length})];
      if (origin) {
        token.origin = origin;
      }
      if (generated) {
    ...
    token(tag, value, {offset, length, origin, data, generated, indentSize} = {}) {
      var token = this.makeToken(tag, value, {offset, length, origin, generated, indentSize});
      if (data) {
    ...
    validateEscapes(str, options = {}) {
      var before, hex, octal, ref, unicode, unicodeCodePoint;
    ...
    error(message, options = {}) {
      var first_column, first_line, ref, ref1;
      var location = 'first_line' in options ? options : ([first_line, first_column] = this.getLineAndColumnFromChunk((ref = options.offset) != null ? ref : 0), {
        first_line,
        first_column,
  ...
  var addTokenData = function(token, data) {
  ...
  var JS_KEYWORDS = ['true', 'false', 'null', 'this', 'new', 'delete', 'typeof', 'in', 'instanceof', 'return', 'throw', 'break', 'continue', 'debugger', 'yield', 'await', 'if', 'else', 'switch', 'for', 'while', 'do', 'try', 'catch', 'finally', 'class', 'extends', 'super', 'import', 'export', 'default'];
  ...
  var IDENTIFIER = /^(?!\d)((?:(?!\s)[$\w\x7f-\uffff])+)([^\n\S]*:(?!:))?/; // Is this a property name?

  // Like `IDENTIFIER`, but includes `-`s
  var JSX_IDENTIFIER_PART = /(?:(?!\s)[\-$\w\x7f-\uffff])+/.source;
  ...
  var JSX_IDENTIFIER = RegExp(`^(?![\\d<])(${JSX_IDENTIFIER_PART // Must not start with `<`.
  // JSXNamespacedName
  // JSXMemberExpression
}(?:\\s*:\\s*${JSX_IDENTIFIER_PART}|(?:\\s*\\.\\s*${JSX_IDENTIFIER_PART})+)?)`);

  // Fragment: <></>
  var JSX_FRAGMENT_IDENTIFIER = /^()>/; // Ends immediately with `>`.
  ...
  var JSX_ATTRIBUTE = RegExp(`^(?!\\d)(${JSX_IDENTIFIER_PART // JSXNamespacedName
  // Is this an attribute with a value?
}(?:\\s*:\\s*${JSX_IDENTIFIER_PART})?)([^\\S]*=(?!=))?`);

  var NUMBER = /^0b[01](?:_?[01])*n?|^0o[0-7](?:_?[0-7])*n?|^0x[\da-f](?:_?[\da-f])*n?|^\d+(?:_\d+)*n|^(?:\d+(?:_\d+)*)?\.?\d+(?:_\d+)*(?:e[+-]?\d+(?:_\d+)*)?/i; // binary
  // octal
  // hex
  // decimal bigint
  // decimal
  ...
  var HERE_JSTOKEN = /^```((?:[^`\\]|\\[\s\S]|`(?!``))*)```/;

  // String-matching-regexes.
  var STRING_START = /^(?:'''|"""|'|")/;

  var STRING_SINGLE = /^(?:[^\\']|\\[\s\S])*/;

  var STRING_DOUBLE = /^(?:[^\\"#]|\\[\s\S]|\#(?!\{))*/;

  var HEREDOC_SINGLE = /^(?:[^\\']|\\[\s\S]|'(?!''))*/;

  var HEREDOC_DOUBLE = /^(?:[^\\"#]|\\[\s\S]|"(?!"")|\#(?!\{))*/;

  var INSIDE_JSX = /^(?:[^\{<])*/; // Start of CoffeeScript interpolation. // Similar to `HEREDOC_DOUBLE` but there is no escaping.
  // Maybe JSX tag (`<` not allowed even if bare).

  var JSX_INTERPOLATION = /^(?:\{|<(?!\/))/; // CoffeeScript interpolation.
  // JSX opening tag.
  ...
  var VALID_FLAGS = /^(?!.*(.).*\1)[gimsuy]*$/;

  var HEREGEX = /^(?:[^\\\/#\s]|\\[\s\S]|\/(?!\/\/)|\#(?!\{)|\s+(?:#(?!\{).*)?)*/; // Match any character, except those that need special handling below.
  ...
  var HEREGEX_COMMENT = /(\s+)(#(?!{).*)/gm;

  var REGEX_ILLEGAL = /^(\/|\/{3}\s*)(\*)/;

  var POSSIBLY_DIVISION = /^\/=?\s/;
  ...
  var STRING_INVALID_ESCAPE = /((?:^|[^\\])(?:\\\\)*)\\(?:(0\d|[1-7])|(x(?![\da-fA-F]{2}).{0,2})|(u\{(?![\da-fA-F]{1,}\})[^}]*\}?)|(u(?!\{|[\da-fA-F]{4}).{0,4}))/; // Make sure the escape isn’t escaped.
  // octal escape
  // hex escape
  // unicode code point escape
  // unicode escape

  var REGEX_INVALID_ESCAPE = /((?:^|[^\\])(?:\\\\)*)\\(?:(0\d)|(x(?![\da-fA-F]{2}).{0,2})|(u\{(?![\da-fA-F]{1,}\})[^}]*\}?)|(u(?!\{|[\da-fA-F]{4}).{0,4}))/; // Make sure the escape isn’t escaped.
  // octal escape
  // hex escape
  // unicode code point escape
  // unicode escape
  ...
  var CALLABLE = ['IDENTIFIER', 'PROPERTY', ')', ']', '?', '@', 'THIS', 'SUPER', 'DYNAMIC_IMPORT'];
